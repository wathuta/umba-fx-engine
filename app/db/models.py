...
# Column names reused by constraints and indexes.
FIELD_BASE_CURRENCY = "base_currency"
FIELD_CURRENCY = "currency"
FIELD_CUSTOMER_ID = "customer_id"
FIELD_QUOTE_CURRENCY = "quote_currency"

# Foreign-key target names.
FK_CUSTOMERS_ID = "customers.id"
FK_RATE_SNAPSHOTS_ID = "rate_snapshots.id"
FK_RATE_REFRESHES_ID = "rate_refreshes.id"
FK_QUOTES_ID = "quotes.id"
...
# New refresh rows start with no updated pairs.
INITIAL_COUNT = 0
...
class Balance(Base):
    ...
    __table_args__ = (UniqueConstraint(FIELD_CUSTOMER_ID, FIELD_CURRENCY, name="uq_balances_customer_currency"),)

    id = uuid_pk()
    customer_id: Mapped[str] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey(FK_CUSTOMERS_ID),
        nullable=False,
        index=True,
    )
...
class RateRefresh(Base):
    ...
    pairs_updated: Mapped[int] = mapped_column(Integer, nullable=False, default=INITIAL_COUNT)
    ...
    duration_ms: Mapped[int] = mapped_column(Integer, nullable=False, default=INITIAL_COUNT)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class RateSnapshot(Base):
    ...
    id = uuid_pk()
    rate_refresh_id: Mapped[str] = mapped_column(UUID(as_uuid=True), ForeignKey(FK_RATE_REFRESHES_ID), nullable=False)
    ...
    mid_rate: Mapped[Decimal] = mapped_column(Numeric(RATE_PRECISION, RATE_SCALE), nullable=False)
    provider: Mapped[str] = mapped_column(String(PROVIDER_NAME_MAX_LENGTH), nullable=False)
    provider_timestamp: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
...
class CurrentRate(Base):
    ...
    __table_args__ = (UniqueConstraint(FIELD_BASE_CURRENCY, FIELD_QUOTE_CURRENCY, name="uq_current_rates_pair"),)
    ...
    rate_snapshot_id: Mapped[str] = mapped_column(UUID(as_uuid=True), ForeignKey(FK_RATE_SNAPSHOTS_ID), nullable=False)
...
class Quote(Base):
    ...
    source_amount: Mapped[Decimal] = mapped_column(Numeric(MONEY_PRECISION, MONEY_SCALE), nullable=False)
    destination_amount: Mapped[Decimal] = mapped_column(Numeric(MONEY_PRECISION, MONEY_SCALE), nullable=False)
    executable_rate: Mapped[Decimal] = mapped_column(Numeric(RATE_PRECISION, RATE_SCALE), nullable=False)
...
class Execution(Base):
    ...
    id = uuid_pk()
    quote_id: Mapped[str] = mapped_column(UUID(as_uuid=True), ForeignKey(FK_QUOTES_ID), nullable=False)
    customer_id: Mapped[str] = mapped_column(UUID(as_uuid=True), ForeignKey(FK_CUSTOMERS_ID), nullable=False)
    debit_currency: Mapped[str] = mapped_column(String(CURRENCY_CODE_LENGTH), nullable=False)
    debit_amount: Mapped[Decimal] = mapped_column(Numeric(MONEY_PRECISION, MONEY_SCALE), nullable=False)
    credit_currency: Mapped[str] = mapped_column(String(CURRENCY_CODE_LENGTH), nullable=False)
    credit_amount: Mapped[Decimal] = mapped_column(Numeric(MONEY_PRECISION, MONEY_SCALE), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
